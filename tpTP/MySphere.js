...
class MySphere extends CGFobject
{
    ...
    addToIndices(v){
        ...
        //console.log("Adding "+Math.floor(this.addToIndices.counterT++/3) +"th index "+v + " to sphere.");
        this.indices.push(v);
    ...
    addToNormals(v){
        ...
        //console.log("Adding normal "+v + " to sphere.");
        this.normals.push(v);
    ...
    addToVertices(v){
        ...
        //console.log("Adding "+Math.floor(this.addToVertices.counterT++/3) +"th vertix "+v + " to sphere.");
    ...
    initBuffers() 
	{
        ...
        let angle2Increment=Math.PI/this.stacks;
        let zInc=1/this.stacks*2;
        let z = 1;
        let angle2=0;
        for(let i = 0; i <= this.stacks; i++){
            ...
            for(let j = 0; j < this.slices; j++){
                this.addToVertices(Math.cos(angle)*Math.sin(angle2));
                this.addToVertices(Math.sin(angle)*Math.sin(angle2));
                this.addToVertices(z*1.6);
                angle+=angleIncrement;
            }
            angle2+=angle2Increment;
            z-=zInc*Math.sin(angle2);
        ...
        for(let i = 0; i <= this.stacks; i++){;
            ...
            for(let j = 0; j < this.slices; j++){
                this.addToNormals(Math.cos(angle)*Math.sin(angle2));
                this.addToNormals(Math.sin(angle)*Math.sin(angle2));
                this.addToNormals(Math.cos(angle2));
                angle+=angleIncrement;
            }
            angle2+=angle2Increment;
            z-=zInc*Math.sin(angle2);
